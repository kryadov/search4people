...
from sqlalchemy import create_engine, String, Integer, Index, func
...
def find_existing_person(first_name: Optional[str] = None, last_name: Optional[str] = None,
                          surname: Optional[str] = None, phone: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    Try to find an existing active person that matches all provided non-empty fields.
    - Name fields are compared case-insensitively (trimmed).
    - Phone is compared as exact string (trimmed).
    Returns the first most recently updated match as a dict, or None if no match.
    """
    with _get_session() as session:
        q = session.query(Person).filter(Person.status == "active")
        conditions = []
        if first_name is not None and str(first_name).strip() != "":
            conditions.append(func.lower(Person.first_name) == str(first_name).strip().lower())
        if last_name is not None and str(last_name).strip() != "":
            conditions.append(func.lower(Person.last_name) == str(last_name).strip().lower())
        if surname is not None and str(surname).strip() != "":
            conditions.append(func.lower(Person.surname) == str(surname).strip().lower())
        if phone is not None and str(phone).strip() != "":
            conditions.append(Person.phone == str(phone).strip())
        if not conditions:
            return None
        q = q.filter(*conditions).order_by(Person.updated_at.desc(), Person.id.desc())
        found = q.first()
